...
class UserViewModel(private val mainInteractor: UserInteractor = UserInteractor()) : ViewModel() {
    ...
    private var user_data:List<User> = listOf()
    ...
    fun bind(userView: UserView) {
        ...
        compositeDisposable.add(mergedIntentsObservable.scan(UserViewState(false,false, user_data),
    ...
    private fun reduce(previousState: UserViewState,partialState: PartialMainViewState): UserViewState {
        return when (partialState) {
            ...
            is PartialMainViewState.ListFetchedState -> UserViewState(userList = user_data)
        }
    }
    ...
    /*private fun renderList(users: Single<List<User>>) : List<User> {
        val user_data1:List<User> = listOf()
        users.let { users -> user_data1}
        return user_data1
    }*/
